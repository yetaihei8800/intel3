...
RUN echo 'LYF123' | vncpasswd -f > $HOME/.vnc/passwd
...
RUN echo 'whoami ' >>/LYF123.sh

RUN echo 'cd ' >>/LYF123.sh

RUN echo "su -l -c 'vncserver :2000 -geometry 1360x768' "  >>/LYF123.sh

RUN echo 'cd /noVNC-1.2.0' >>/LYF123.sh

RUN echo './utils/launch.sh  --vnc localhost:7900 --listen 8900 ' >>/LYF123.sh

RUN chmod 755 /LYF123.sh
